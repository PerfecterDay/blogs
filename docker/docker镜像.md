...
添加目录以外的路径到 `buildcontext` 下：
```
docker build --build-context modu=/Users/gtja/workspace/apps/go-modu --build-arg env=staging -t gtja-registry-registry.cn-hongkong.cr.aliyuncs.com/staging/ttl-gmt:1.0.0 .
```

`Dockerfile` 中使用： `COPY --from=modu . /usr/local/gtja/com_gmas/go-modu/`
