...
SHF_WRITE</td>
<td
style="border: 1px solid; padding: 0.2rem 1.5rem">此节包含全局偏移量表</td>
</tr>
<tr class="even">
...
<td style="border: 1px solid; padding: 0.2rem 1.5rem">SHT_PROGBITS</td>
<td style="border: 1px solid; padding: 0.2rem 1.5rem">见下文</td>
<td style="border: 1px solid; padding: 0.2rem 1.5rem">此节含有 ELF
程序解析器的路径名。如果此节被包含在某个可装载的段中，
那么本节的属性中应置 SHF_ALLOC 标志位，否则不置此标志。</td>
</tr>
<tr class="even">
...
SHF_EXECINSTR</td>
<td
style="border: 1px solid; padding: 0.2rem 1.5rem">此节包含函数连接表</td>
</tr>
<tr class="even">
<td
style="border: 1px solid; padding: 0.2rem 1.5rem">.relname/.relaname</td>
<td
...
的属性中应置 SHF_ALLOC 标志位，否则不置此标志。注意，这两个节的名字
中”name”是可替换的部分，执照惯例，对哪一节做重定位就把”name”换成哪一节
的名字。比如，.text 节的重定位节的名字将是.rel.text 或.rela.text。</td>
</tr>
<tr class="odd">
<td
...
8. pax-utils-`apt install pax-utils`
...
9. [Radare2](https://www.radare.org/r/)
10. elfutils -`sudo apt install elfutils -y`
