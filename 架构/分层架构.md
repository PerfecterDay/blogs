...
> https://bootify.io/multi-module/spring-modulith-vs-multi-module.html  
...
只做三件事：
+ 参数接收 & 校验
+ DTO ↔ Command/Query 转换，DTO 转换为业务对象
+ 调用 application 层
...
### 第三方接口调用
调用第三方接口，成功返回A类型对象，失败返回B类型对象，这种方法如何设计 ？
1. 失败时 抛出一个异常
2. 设计一个返回类型，包含 success 字段来表示请求的成功与失败， result/fault 对象分别表示成功和失败的结果

选用2，异常用于调用失败（连接失败、超时等场景）


### 事件处理
当一个业务完成后，需要做多个处理通知相关系统或执行多个操作时，使用事件通知机制。 比如用户绑定一个设备后，需要邮件通知客户，需要通知另一个登录系统，还需要调用用户中心的接口。
```
// ❌ 不推荐
@EventListener
...
```
@Component
class TokenBoundEmailHandler {

    @EventListener
    void on(TokenBoundSuccessEvent e) {
        emailService.send(e);
    }
}

@Component
class TokenBoundUcHandler {

    @EventListener
    void on(TokenBoundSuccessEvent e) {
        userCenterService.notify(e);
    }
}
...
class ClientAuthHandler {
    ...
    @EventListener
    void on(TokenBoundSuccessEvent e) {
        clientAuthCLient.sync(e);
    }
}
```

### sevice 层是否要返回 VO 对象
不要返回， Controller 层的对象，返回业务对象